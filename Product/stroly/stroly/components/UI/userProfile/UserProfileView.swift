...
struct DecorationSettings {
    ...
    // カテゴリのアイコンの大きさ
    let categoryIconWidth: CGFloat = UIScreen.main.bounds.width / 6
    let categoryIconHeight: CGFloat = UIScreen.main.bounds.width / 6
}
...
//                }
            ...
            .fullScreenCover(isPresented: $isGenQRButtonTapped)
            {
                QRSheetView(isScanButtonTapped: $isScanButtonTapped, QRImage: $qrimage)
            }
            .onDisappear {
            ...
            VStack {
                ...
                HStack {
                    ...
                    .alert(isPresented: $isShowingAlert) {
                        Alert(title: Text("QRコードについて"), message: Text("お互いのQRコードを読み取りあうことで友達登録ができます！"), dismissButton: .default(Text("OK")))
                    }
                    Spacer()
                        .frame(width: decoSet.sideWidth)
                }
